...
async function clearSheets() {
  ...
  await sheetsWrite(() =>
    sheets.spreadsheets.values.clear({
      spreadsheetId,
      range: 'campanhas!A2:Z',
    })
  );
  console.log('✔ Aba "campanhas" limpa');

  await sheetsWrite(() =>
    sheets.spreadsheets.values.clear({
      spreadsheetId,
      range: 'unfinished!A2:Z',
    })
  );
  console.log('✔ Aba "unfinished" limpa');
}
...
export async function main() {
  ...
  try {
    ...
    console.log('→ Etapa 0: Limpando planilhas...');
    await clearSheets();
    console.log('✔ Planilhas limpas\n');
